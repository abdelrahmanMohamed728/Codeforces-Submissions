...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean check = false;
        if (n>=k) {
            int min = Math.min(k-1,n-1);
            for (int i =min; i >= 0; i--) {
                if (!check&&s.charAt(i) ==a[n-1])
                    t.append((char)a[0]);
                else {
                    if (check)
                        t.append( s.charAt(i));
                    ...
                    {
                        t.append((char)a[getIndex(a,n,s.charAt(i))+1]);
                        check = true;
                    }
                }
            }
    ...
    static int getIndex(int[]a,int n,int item)
    {
        int index =-1;
        for (int i =0;i<n;i++)
            if (a[i]==item)
...
class LazySegmentTree {
    final int MAX = 400001;        // Max tree size
    ...
    /*  us and eu -> starting and ending indexes of update query
        ue  -> ending index of update query
        diff -> which we need to add in the range us to ue */
    void updateRange(int n, int us, int ue, long diff) {
