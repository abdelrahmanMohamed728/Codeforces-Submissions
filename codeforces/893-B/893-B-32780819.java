...
public class Main {
    public static void main(String[] args) {
        ...
        int n = in.readInt();
        ArrayList<Integer> Beautiful = getBeautiful();
ArrayList<Integer> Divisors = getDivisors(n);
Collections.sort(Divisors);
int ans=1;
for (int i =Divisors.size()-1 ; i>=0; i--)
{
   if (Beautiful.contains(Divisors.get(i)))
   {
       ans = Divisors.get(i);
       break;
   }
...
static ArrayList<Integer> getBeautiful()
{
    ArrayList<Integer> a = new ArrayList<>();
    for (int i =1;i<=9;i++)
    {
        a.add((int)(Math.pow(2,i)-1)*(int)Math.pow(2,i-1));
    }
    return a;
}
